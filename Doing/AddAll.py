...
def set_fiber(n):
    if n == 1:
        return a.instances['Part-fiber-0'].edges[0:1]
    else:
        return a.instances['Part-fiber-{}'.format(n - 1)].edges[0:1] + set_fiber(n - 1)


def set_aggregate(n):
    if n == 1:
        return a.instances['sphere0'].cells[0:1]
    else:
        return a.instances['sphere{}'.format(n-1)].cells[0:1] + set_aggregate(n - 1)
...
mdb.models['Model-1'].materials['Material-Matrix'].conductivity.setValues(
    temperatureDependency=ON, table=((2.0, 20.0), (1.6, 300.0), (1.5, 400.0), (1.4, 500.0)))
...
mdb.models['Model-1'].materials['Material-fiber'].SpecificHeat(table=((500,),))
...
file_fibers = 'E:\Abaqus\Code\CycleInputFile\data_fibers.txt'
# 生成骨料
a = mdb.models['Model-1'].rootAssembly
...
# 生成纤维
bf = np.loadtxt(file_fibers, delimiter=',', dtype=np.float32)
countf = len(open(file_fibers, 'rU').readlines())
...
# 定义纤维集合
a = mdb.models['Model-1'].rootAssembly
edgesFibers = set_fiber(countf)
# 定义骨料集合
cellsAggregate = a.instances['sphere0'].cells[0:1]
cellsAggregate = set_aggregate(countg)
# 定义基质集合
...
p.seedPart(size=0.00625, deviationFactor=0.1, minSizeFactor=0.1)
p.generateMesh()
# fiber mesh
p = mdb.models['Model-1'].parts['fibers']
session.viewports['Viewport: 1'].setValues(displayedObject=p)
