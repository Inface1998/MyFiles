...
"""
	double node[NN][6]={0.0};	//node[][0]-node number; 
								//node[][1-3]-node coordinates; 
								//node[][4]-node temperature of this time step; 
								//node[][5]-node vapor pressure(MPa) 

	int  elem[EN][6]={0};       //elem[][0]-original elem number; 
								//elem[1-3]-element node;
								//elem[][4]-elem material type 1-aggregate 0-matrix;
								//elem[][5]-renumbered element number (the eliminated element number is 0)
	...
	double elemp[EN][]={0.0};	//elemp[][0]-elem number;
								...
								//elemp[][6]-
								//elemp[][7]-specific volume of vapor and liquid water mixture(m3/kg)(before mass)
								//elemp[][8]-element vapor presure(MPa);
								//elemp[][9,10,11]-damage parameters and the damage angle in the local coordinate-system(D1e,D2e,Q)
								//elemp[][12]-the type of vapor,0-water,1-saturated water,2-superheated steam,3-presure is beyond 35Mpa,4-data gap
                                //-elemp[][13]-material strength (MPa)
                                //-elemp[][14]-damage parameter of E_modulus by dehydration/decomposition

	double elemdh[EN][]={0.0};//elemdh[][0]-the total amount of moisture transported per unit volume of hcp before this time step(Kg/m3)
								...
								//elemdh[][7]-initial volume fraction of capillary pores;
								...
								//elemdh[][18]-the mass of liquid and vapor mixture per unit volume of hcp(g/cm3); 
								//elemdh[][19]-porosity increased by full dehydration(the degree of dehydration=1.0 );
								//elemdh[][20]-degree of dehydration=porosity increased by dehydration/porosity increased by full dehydration
												//the value is between 0.0~1.0
								//elemdh[][21]-element critical pore diameter (m)
								//elemdh[][22]-the volume fraction of residual unhydrated blender after dehydration
    ...
    double elempe[EN][]={0.0};	//elempe[][1]-level 1 permeability;
								...
								//elempe[][6]-element relative permeability, the value is between 1 and zero.
								//elempe[][7]-the element effective gas permeability (taken the relative permeability into account)(m2)
								//elempe[][8]-element poisson's ratio

    double elemvp[EN][]={0.0};//elemvp[][1]-element vapor (gas phase) specific volume (m3/kg)
								//elemvp[][2]-element vapor compresibility factor Z (dimensionless)
								//elemvp[][3]-dynamic viscosity of vapor (N*S/m2)
								//elemvp[][4]-element equivalent density for mass transport analysis
								//elemvp[][5]-element equivalent thermal capacity for mass transport analysis
								//elemvp[][6]-element equivalent thermal conductivity (permeability) for mass transport analysis
								//elemvp[][7]-element vapor slippage flow factor b (MPa)0
								//elemvp[][8]-liquid water saturation degree for relative permeability calculation
								//elemvp[][9]-M/ZRT for the mass migration analysis in the mechanical analysis
								//elemvp[][10] --element vapor pressure drop because of mass transport (MPa)
								//elemvp[][11] --vapor density change because of mass transport (kg/m3)
								//elemvp[][12] --specific volume of vapor and liquid water mixture(m3/kg)(after mass)
								//elemvp[][13] --the amount of mass change per unit volume of hcp of this time step (kg/m3)
								//elemvp[][14] --the total amount of mass transported per unit volume of hcp after this time step (kg/m3)
								//elemvp[][15] --the total amount of mass transported per unit volume of hcp before this time step analysis  (kg/m3)
								//elemvp[][16] --moisture content,if the moisture is converted to liquid water,the volume fraction of capillary pores filled with water


	double wv[34][11];			//saturated water(liquid-vapor), see "Properties of Saturated Water.pdf"
								//if element temperature is below 100c, there's no vapor pressure.	  

	double sw[26][36]={0.0};    //superheated steam,sw[1-24][0]-the number of specific volum; sw[][1]-temperature; sw[][2-35]-specific volum; 
								//sw[0][2-35]-pressure, sw[0][0]-the number of pressure,sw[25][]-store temperary interpolation data

	double swv[26][36]={0.0};	//the viscosity of superheated steam, the structure is the same as sw[][];
"""
...
def output_massVapor(odb_name, excel_name):
    wb = openpyxl.load_workbook(excel_name)
    sheet1 = wb["MatrixNode"]
    sheet2 = wb["MatrixElement"]
    sheet1.cell(1, 6, "VaporForMass")
    sheet2.cell(1, 15, "VaporForMass")
    ...
    center = odb.rootAssembly.nodeSets['SET-MATRIX']
    # 创建变量centerDisplacement，得到region center的位移场数据
    centerDisplacement = displacement.getSubset(region=center)
    for i in range(MN):
        # sheet1.cell(i + 2, 1, centerDisplacement.values[i].nodeLabel)
        if centerDisplacement.values[i].data < 0.1:
            centerDisplacement.values[i].data == 0.1
        sheet1.cell(i + 2, 6, centerDisplacement.values[i].data)
    for i in range(ME):
        node_vapor = []
        for j in range(2, 6):
...
# 读取第5秒末不同部件的单元和节点温度信息并输出
def output_temperature(odb_name, inp_name, excel_name):
    wb = openpyxl.Workbook()
    ws1 = wb.create_sheet("MatrixNode", 0)
    ws1.cell(1, 1, "M-NodeNum")
    ws1.cell(1, 2, "M-NodeCoordx")
    ws1.cell(1, 3, "M-NodeCoordy")
    ws1.cell(1, 4, "M-NodeCoordz")
    ws1.cell(1, 5, "M-NodeTemp")
    ws2 = wb.create_sheet("MatrixElement", 1)
    ws2.cell(1, 1, "M-ElemNum")
    ws2.cell(1, 2, "M-ElemNode1")
    ws2.cell(1, 3, "M-ElemNode2")
    ws2.cell(1, 4, "M-ElemNode3")
    ws2.cell(1, 5, "M-ElemNode4")
    ws2.cell(1, 6, "M-ElemTemp")
    ws2.cell(1, 7, "M-ElemCoordx")
    ws2.cell(1, 8, "M-ElemCoordy")
    ws2.cell(1, 9, "M-ElemCoordz")
    odb = openOdb(path=odb_name)
    lastFrame = odb.steps['Step-1'].frames[-1]  # 创建变量lastFrame，得到载荷步Step-1的最后一帧
    displacement = lastFrame.fieldOutputs['NT11']  # 创建变量displacement，得到最后一帧的温度场数据
    # 其中Part-cut为装配体中的几何体，SET-为创建的集合
    ...
    centerDisplacement = displacement.getSubset(region=center)
    for i in range(MN):
        ws1.cell(i + 2, 1, centerDisplacement.values[i].nodeLabel)
        ws1.cell(i + 2, 5, centerDisplacement.values[i].data)
        # 导入基质节点坐标
        a = mdb.models['Model-1'].parts['Part-cut'].nodes[i].coordinates
        ws1.cell(i + 2, 2, a[0])
        ws1.cell(i + 2, 3, a[1])
        ws1.cell(i + 2, 4, a[2])
    with open(inp_name, 'r') as fr:
        n = 1
        logging.info(MN)
        logging.info(ME)
        for line in fr.readlines()[MN + 10:MN + 10 + ME]:
            text = line.split()
            EN = float(text[0].split(',')[0])
            A = float(text[1].split(',')[0])
            B = float(text[2].split(',')[0])
            C = float(text[3].split(',')[0])
            D = float(text[4].split(',')[0])
            ws2.cell(n + 1, 1, EN)
            ws2.cell(n + 1, 2, A)
            ws2.cell(n + 1, 3, B)
            ws2.cell(n + 1, 4, C)
    ...
    sheet1 = wb2["MatrixNode"]
    sheet2 = wb2["MatrixElement"]
    for i in range(ME):
        node_temp = []
        node_x = []
        node_y = []
        ...
        sheet2.cell(i + 2, 6, 0.25 * (node_temp[0] + node_temp[1] + node_temp[2] + node_temp[3]))
        sheet2.cell(i + 2, 7, 0.25 * (node_x[0] + node_x[1] + node_x[2] + node_x[3]))
        sheet2.cell(i + 2, 8, 0.25 * (node_y[0] + node_y[1] + node_y[2] + node_y[3]))
        sheet2.cell(i + 2, 9, 0.25 * (node_z[0] + node_z[1] + node_z[2] + node_z[3]))
    wb2.save(excel_name)
    odb.close
...
def calc_decomposition(excel_name, last_excel, dehy_name):
    wb1 = openpyxl.load_workbook(last_excel)
    wb2 = openpyxl.load_workbook(excel_name)
    sheet1 = wb1["MatrixElement"]
    sheet2 = wb2["MatrixElement"]
    for i in range(ME):
        if step == 0:
            elemp[i][4] = 20
        else:
            elemp[i][4] = sheet1.cell(i + 2, 6).value
        # 计算单元平均温度以用于热分解计算
        elemp[i][5] = sheet2.cell(i + 2, 6).value
        elemp[i][3] = 0.5 * (elemp[i][4] + elemp[i][5])
        # AFt decomposition
        if (elemp[i][3] > 70.0):
            if (elemdh[i][8] < 1.0):
                elemdh[i][8] = elemdh[i][8] + 1e6 / 60.0 * math.exp(-7.0965e3 / (elemp[i][3] + 273.15)) * math.sqrt(
                    1.0 - elemdh[i][8]) * TG
            if (elemdh[i][8] > 1.0):
                elemdh[i][8] = 1.0
        # CH decomposition
        ...
        if (elemp[i][3] >= 600.0 and elemp[i][3] <= 700.0):
            ...
            if (elemdh[i][10] >= 0.9625):
                ...
                t = 0.0
                for i in range(30):
                    t1 = t - (-2.0e-8 * pow(t, 5) + 8.0e-6 * pow(t, 4) - 0.0015 * pow(t, 3) + 0.138 * pow(t,2) - 5.47 * t + 99.82 - ra * 100) / (
            ...
            if (elemdh[i][10] >= 0.9625):
                ...
                t = 0.0
                for i in range(30):
                    t1 = t - (-2.0e-8 * pow(t, 5) + 8.0e-6 * pow(t, 4) - 0.0015 * pow(t, 3) + 0.138 * pow(t,2) - 5.47 * t + 99.82 - ra * 100.0) / (
                                 -1.0e-7 * pow(t, 4) + 3.2e-5 * pow(t, 3) - 0.0045 * pow(t, 2) + 0.26 * t - 5.47)
                    if (abs((t1 - t) / t1) <= ERRO):
                        break
                    else:
                        t = t1
                t800 = t1
                d800 = (1.0e-7 * pow(t800, 4) - 3.2e-5 * pow(t800, 3) + 0.0045 * pow(t800,2) - 0.26 * t800 + 5.47) / 100
            if (elemdh[i][10] < 1.0):
    ...
    wb = openpyxl.Workbook()
    ws1 = wb.create_sheet("conversion_degree", 0)
    ws1.cell(1, 1, "elem")
    ws1.cell(1, 2, "AFt_cd")
    ws1.cell(1, 3, "CH_cd")
    ws1.cell(1, 4, "CSH_cd")
    ...
    if step*TG*TV < 100 :
        ...
        for i in range(ME):
            ...
            if elemp[i][7] != 0.0:
                ...
                # 考虑超出过热蒸汽表范围的单元蒸汽压
                if elemp[i][8] == -1.0:
                    elemp[i][12] = 3.0  # 赋予状态为超过热蒸汽压
                    elemp[i][8] = 35.0  # 超过热蒸汽压全部为35Mpa
                    elemvp[i][1] = elemp[i][7]  # 赋予比体积
                    elemvp[i][8] = 0.0  # 液态水饱和度
                    for k in range(1, 34):
                        if sheet1.cell(k + 1, 1).value < elemp[i][3] < sheet1.cell(k + 2, 1).value:
                            elemp[i][8] = interp(sheet1.cell(k + 1, 2), sheet1.cell(k + 2, 2), sheet1.cell(k + 1, 1),
...
def calc_mass(mass_name, jobName, massCae):
    if step*TG*TV < 100 :
        return
    # 输出质量传输的相关参数到massMigration-1.xlsx
    wb = openpyxl.Workbook()
    ws1 = wb.create_sheet("MassTrans", 0)
    ws1.cell(1, 1, "ElemNum")
    ws1.cell(1, 2, "EquDensity")  # elemvp[i][4]
    ws1.cell(1, 3, "EquThermCapa")  # elemvp[i][5]
    ws1.cell(1, 4, "EquTermCon")  # elemvp[i][6]
    ws1.cell(1, 5, "FraCapillary")  # elemdh[i][17]
    ws1.cell(1, 6, "SpeVolume")  # elemp[i][7]
    ws1.cell(1, 7, "LiqWaterSat")  # elemvp[i][8]
    ws1.cell(1, 8, "EModule")  # elempe[i][4]
    ws1.cell(1, 9, "M/ZRT")  # elemvp[][9]
    for i in range(ME):
        # =============================计算基质的质量传输相关参数===================================
        # 计算压缩性因素Z
        ...
        # 考虑汽相滑移的渗透性
        elempe[i][5] = elempe[i][3] * (1.0 + elemvp[i][7] / (
                elemp[i][8] * 10.0))  # *10.0 is to convert the vapor pressure from MPa to atmosphare
        # 相对渗透性系数
        rr = 0.05 - 22.5 * elemdh[i][17]
        elempe[i][6] = pow(10, elemvp[i][8] * rr) - pow(10, rr) * elemvp[i][8]
        # 单元有效渗透性
        elempe[i][7] = elempe[i][5] * elempe[i][6]
        elemvp[i][4] = 1.0 / 0.4614  # 质量传输分析的元素等效密度  R=0.4614kJ/(kg*K)
        elemvp[i][5] = 1.0 / elemvp[i][2] / (elemp[i][3] + 273.15) * elemdh[i][17] * (
                1.0 - elemvp[i][8])  # 质量传输分析的元素等效热容
        elemvp[i][9] = 1.0 / (0.4614 * elemvp[i][2] * (elemp[i][3] + 273.15))  # M/ZRT
        elemvp[i][6] = elempe[i][7] / elemvp[i][3] / elemvp[i][1] * 1e3  # 等效热传导率计算
        # 输出相关信息到指定文件
        ws1.cell(i + 2, 1, i + 1)
        ws1.cell(i + 2, 2, elemvp[i][4])
        ws1.cell(i + 2, 3, elemvp[i][5])
        ws1.cell(i + 2, 4, elemvp[i][6])
        ws1.cell(i + 2, 5, elemdh[i][17])
        ws1.cell(i + 2, 6, elemp[i][7])
        ws1.cell(i + 2, 7, elemvp[i][8])
        ws1.cell(i + 2, 8, elempe[i][4])
        ws1.cell(i + 2, 9, elemvp[i][9])
    wb.save(mass_name)
    wb = openpyxl.load_workbook(mass_name)
    ws1 = wb["MassTrans"]
    # 定义基质的等效属性
    mdb.Model(name='Model-Mass-1', objectToCopy=mdb.models['Model-1'])
    model = mdb.models['Model-Mass-1']
    myPart = model.parts['Part-cut']
    myElement = myPart.elements
    for i in range(ME):
        materialName = "Material-M-{}".format(i + 1)
        sectionName = "Section-M-{}".format(i + 1)
        model.Material(name=materialName)
        model.materials[materialName].Density(table=((ws1.cell(i + 2, 2).value,),))
        model.materials[materialName].SpecificHeat(table=((ws1.cell(i + 2, 3).value,),))
        model.materials[materialName].Conductivity(table=((ws1.cell(i + 2, 4).value,),))
        model.HomogeneousSolidSection(name=sectionName, material=materialName, thickness=None)
        setName = 'SET-{}'.format(i + 1)
        myPart.Set(elements=myElement[i:i + 1], name=setName)
        myPart.SectionAssignment(region=myPart.sets[setName], sectionName='Section-M-{}'.format(i + 1),
                                 offset=0.0, offsetType=MIDDLE_SURFACE, offsetField='',
                                 thicknessAssignment=FROM_SECTION)
    # =============================输入骨料的质量传输相关参数===================================
    # 为所有非基质单元赋予质量传输分析的元素等效热容：1/(elemp[i][3]+273.15)*1e2
    # 等效热导率(渗透率)：0.0
    mdb.models['Model-Mass-1'].materials['Material-Aggregate'].conductivity.setValues(table=((0.0,),))
    # 撤除相互作用
    ...
    f1 = a.instances['Part-cut-1'].faces
    faces1 = f1[0:6]
    ...
    # 删除内置区域
    mdb.models['Model-Mass-1'].constraints['Constraint-fibers'].suppress()
    # 定义预定义场
    del mdb.models['Model-Mass-1'].predefinedFields['Predefined Field-1']
    # 增量步修改
    mdb.models['Model-1'].steps['Step-1'].setValues(minInc=1e-08, deltmx=1.0)
    # 定义映射场
    excel_name = 'E:/Abaqus/Code/CycleOutputFile/NodeElement-1.xlsx'
    wb = openpyxl.load_workbook(excel_name)
    sheet = wb["MatrixElement"]
    vapor_point = ()
    for i in range(ME):
        vapor_point += ((sheet.cell(i + 2, 7).value, sheet.cell(i + 2, 8).value, sheet.cell(i + 2, 9).value,
    ...
    c1 = a.instances['Part-cut-1'].cells
    cells1 = c1[0:1]
    region = regionToolset.Region(cells=cells1)
    mdb.models['Model-Mass-1'].Temperature(name='Initial-vapor',
    ...
    for i in range(countg):
        mdb.models['Model-Mass-1'].constraints['CP-{}-Part-cut-1-sphere{}'.format(i + 1, i)].suppress()
...
# 计算累计湿传输
def calc_moist(excel_name):
    # sheet3 = wb3["MassTrans"]
    wb2 = openpyxl.load_workbook(excel_name)
    sheet2 = wb2["MatrixElement"]
    # 计算湿传输后的比体积
    for i in range(ME):
        elemvp[i][10] = sheet2.cell(i + 2, 12).value - sheet2.cell(i + 2, 10).value  # 蒸气压变化值
        if math.fabs(elemvp[i][10]) < 1e-10:
            elemvp[i][10] = 0.0
        elemvp[i][11] = elemvp[i][10] * elemvp[i][9] * 1e3  # 蒸汽密度变化值
        elemvp[i][12] = elemp[i][7] / (1 + elemp[i][7] * (1.0 - elemvp[i][8]) * elemvp[i][11])  # 湿传输后比体积计算
        elemp[i][7] = elemvp[i][12]
        elemvp[i][13] = (1.0 - elemvp[i][8]) * elemdh[i][17] * elemvp[i][11]  # 此分析步中的质量传输
        elemvp[i][14] = elemvp[i][15] + elemvp[i][13]  # 此分析步末的累计质量传输
        if elemp[i][7] != 0:
            elemvp[i][16] = 1.0 / elemp[i][7] / 1e3
        if elemvp[i][16] > 1.0:
            elemvp[i][16] = 1.0
        # 输出相关信息
    wb1 = openpyxl.load_workbook(mass_name)
    sheet1 = wb1["MassTrans"]
    sheet1.cell(1, 10, "ThisMT")
    sheet1.cell(1, 11, "TotalMT")
    for i in range(ME):
        sheet1.cell(i + 2, 10, elemvp[i][13])
        sheet1.cell(i + 2, 11, elemvp[i][14])
    wb1.save(mass_name)
    wb2.save(excel_name)
...
if __name__ == '__main__':
    logging.basicConfig(level=logging.INFO, filename='./log.txt',
                        filemode='w', format='%(asctime)s - %(filename)s[line:%(lineno)d] - %(levelname)s: %(message)s')
    ...
    A = 2686  # 基质材料本构关系中的参数
    # 读取基质节点和单元个数
    logging.info("openmdb-begin")
    mdb = openMdb(pathName='E:/Abaqus/Workpace/Model-temp01.cae')
    logging.info("openmdb-successs")
    a1 = mdb.models['Model-1'].rootAssembly.instances['Part-cut-1']
    # matrix node and element number
    MN = len(a1.nodes.getByBoundingBox(-1, -1, 0, 1, 1, 1))
    ME = len(a1.elements.getByBoundingBox(-1, -1, 0, 1, 1, 1))
    # 定义数组
    elem = [[0 for i in range(6)] for j in range(ME)]
    elemdh = [[0.0 for i in range(23)] for j in range(ME)]
    elemp = [[0.0 for i in range(15)] for j in range(ME)]
    elempe = [[0.0 for i in range(9)] for j in range(ME)]  # 渗透率相关
    elemvp = [[0.0 for i in range(17)] for j in range(ME)]  # 动力粘度相关
    ...
    for step in range(TS):
        lastTempOdb = 'E:\\Abaqus\\Workpace\\Job-temp0{}.odb'.format(step)
        lastTempCae = 'E:\\Abaqus\\Workpace\\Model-temp0{}.cae'.format(step)
        thisTempCae = 'E:\\Abaqus\\Workpace\\Model-temp0{}.cae'.format(step + 1)
        thisMassCae = 'E:\\Abaqus\\Workpace\\Model-Mass0{}.cae'.format(step + 1)
        if step == 0:
        ...
        # 更改温度幅值
        # (0.0, 345*math.log(8*5*step/60.0+1,10)+20),(5.0, 345*math.log(8*5*(step+1)/60.0+1,10)+20)
        mdb.models['Model-1'].amplitudes['Amp-1'].setValues(timeSpan=STEP,
                                                            smooth=SOLVER_DEFAULT, data=(
                (0.0, 20 + step * TG * TV), (TG, 20 + (step + 1) * TG * TV)))
        submit_job(step, thisTempCae)
        # 读取单步末基质节点温度
        odb_temp = "Job-temp0{}.odb".format(step + 1)
        excel_name = "E:/Abaqus/Code/CycleOutputFile/NodeElement-{}.xlsx".format(step + 1)
        inp_name = "E:/Abaqus/Workpace/Job-temp0{}.inp".format(step + 1)
        output_temperature(odb_temp, inp_name, excel_name)
        # 热分解计算
        last_excel = "E:/Abaqus/Code/CycleOutputFile/NodeElement-{}.xlsx".format(step)
        dehy_name = "E:/Abaqus/Code/CycleOutputFile/dehy-{}.xlsx".format(step + 1)
        if step == 0:
            calc_decomposition(excel_name, excel_name, dehy_name)
        else:
            calc_decomposition(excel_name, last_excel, dehy_name)
        # 蒸气压计算
        calc_vapor(False)
        # 质量传输确定
        mass_name = "E:\Abaqus\Code\CycleOutputFile\massMigration-{}.xlsx".format(step + 1)
        jobName = "Job-mass0{}".format(step + 1)
        calc_mass(mass_name, jobName, thisMassCae)
        # 读取传输后的蒸气压
        odb_mass = "Job-mass0{}.odb".format(step + 1)
        output_massVapor(odb_mass, excel_name)
        calc_moist(excel_name)
        # 再次计算蒸气压
        calc_vapor(True)
    # DONE
    # DONE
