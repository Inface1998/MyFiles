...
wb = openpyxl.load_workbook("/Code/CycleOutputFile/NNE.xlsx")
with open("/Code/CycleInputFile/OdbText.txt", "w+") as f:
    f.write("** odbText.txt\n")
    # 写入Node信息
    f.write("*Node\n")
    sheet1 = wb["AllNode"]
    ...
    for i in range(10):
        ...
        while True:
            ...
            nodeX = str(s1.cell(count, 2).value)
            nodeY = str(s1.cell(count, 3).value)
            nodeZ = str(s1.cell(count, 4).value)
            data =  str(s1.cell(count, 5).value)
            if label == "None":
                f.write("\n")
                break
            else:
                f.write('%-8s%-18s%-18s%-18s%-18s' % (label+",",nodeX+",",nodeY+",",nodeZ+",", data) + '\n')
                count = count + 1
        wb2.close()
wb.close()
